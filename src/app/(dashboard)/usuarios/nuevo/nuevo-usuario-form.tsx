...
import { createUsuarioAction, type UsuarioFormState, type SedeCheckboxOption } from "@/app/actions/usuario-actions";
import { usuarioCreateInputSchema } from "@/lib/validation/usuario";
...
type NuevoUsuarioFormInput = z.input<typeof usuarioCreateInputSchema>;
type NuevoUsuarioFormOutput = z.output<typeof usuarioCreateInputSchema>;
...
export function NuevoUsuarioForm({
   ...
   * NuevoUsuarioDialog) -- defaults to false because this same form is also
   * rendered standalone on /usuarios/nuevo with no Dialog ancestor, where
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<NuevoUsuarioFormInput, unknown, NuevoUsuarioFormOutput>({
      ...
      <FormGroup label="Sedes asignadas">
        <div className="flex flex-col gap-4">
          {esAdmin ? (
            ...
            <div className="flex flex-col gap-1.5">
              {sedes.map((sede) => {
                const inputId = `sede-${sede.id}-nuevo-usuario`;
                return (
